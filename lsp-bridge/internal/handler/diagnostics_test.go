...
func TestCheckMetadataComponentDir_Skipped(t *testing.T) {
	idx, _ := index.New("")
	idx.SetBasePath("/project/stacks")

	// metadata.component is always a definition, never a reference.
	// All checks (path traversal + existence) should be skipped.
	sf := &index.StackFile{
